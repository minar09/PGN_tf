...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror=False):
    ...
    if input_size is not None:
        ...
        img, label, edge = random_crop_and_pad_image_and_labels(
            img, label, edge, h, w, IGNORE_LABEL)
...
class ImageReaderPGN(object):
    ...
    def __init__(self, data_dir, data_list, data_id_list, input_size, random_scale,
        ...
        self.image_list, self.label_list, self.label_rev_list = read_labeled_image_reverse_list(
            self.data_dir, self.data_list)
        self.edge_list = read_edge_list(self.data_dir, self.data_id_list)
        self.images = tf.convert_to_tensor(self.image_list, dtype=tf.string)
        self.labels = tf.convert_to_tensor(self.label_list, dtype=tf.string)
        self.labels_rev = tf.convert_to_tensor(
            self.label_rev_list, dtype=tf.string)
        self.edges = tf.convert_to_tensor(self.edge_list, dtype=tf.string)
        self.queue = tf.train.slice_input_producer(
            [self.images, self.labels, self.labels_rev, self.edges], shuffle=shuffle)
        self.image, self.label, self.edge = read_images_from_disk(
