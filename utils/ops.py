...
def conv2d(input_, output, kernel, stride, relu, bn, name, stddev=0.01):
    with tf.variable_scope(name) as scope:
        ...
        w = tf.get_variable(
            'w', shape, initializer=tf.truncated_normal_initializer(stddev=stddev))
        conv = tf.nn.conv2d(input_, w, strides=[
                            1, stride, stride, 1], padding='SAME')
        # Add the biases
        b = tf.get_variable(
            'b', [output], initializer=tf.constant_initializer(0.0))
...
def linear(input_, output, name, stddev=0.02, bias_start=0.0):
    ...
    with tf.variable_scope(name) as scope:
        ...
        bias = tf.get_variable(
            "bias", [output], initializer=tf.constant_initializer(bias_start))
...
def gcn(input_, output, kernel, stride, relu, bn, name, stddev=0.01):
    with tf.variable_scope(name) as scope:
        ...
        w1_1 = tf.get_variable(
            'w1_1', left_shape_k_1, initializer=tf.truncated_normal_initializer(stddev=stddev))
        w1_2 = tf.get_variable(
            'w1_2', left_shape_1_k, initializer=tf.truncated_normal_initializer(stddev=stddev))
        w2_1 = tf.get_variable(
            'w2_1', right_shape_1_k, initializer=tf.truncated_normal_initializer(stddev=stddev))
        w2_2 = tf.get_variable(
            'w2_2', right_shape_k_1, initializer=tf.truncated_normal_initializer(stddev=stddev))
        b1_1 = tf.get_variable(
            'b1_1', [output], initializer=tf.constant_initializer(0.0))
        b1_2 = tf.get_variable(
            'b1_2', [output], initializer=tf.constant_initializer(0.0))
        b2_1 = tf.get_variable(
            'b2_1', [output], initializer=tf.constant_initializer(0.0))
        b2_2 = tf.get_variable(
            'b2_2', [output], initializer=tf.constant_initializer(0.0))

        conv1_1 = tf.nn.conv2d(input_, w1_1, strides=[
                               1, stride, stride, 1], padding='SAME')
        ...
        conv1_2 = tf.nn.conv2d(conv1_1, w1_2, strides=[
                               1, stride, stride, 1], padding='SAME')
        ...
        conv2_1 = tf.nn.conv2d(input_, w2_1, strides=[
                               1, stride, stride, 1], padding='SAME')
        ...
        conv2_2 = tf.nn.conv2d(conv2_1, w2_2, strides=[
                               1, stride, stride, 1], padding='SAME')
...
def br(input_, output, kernel, stride, name):
    with tf.variable_scope(name) as scope:
        ...
        br_conv1 = conv2d(input_, output, kernel, stride,
                          relu=True, bn=False, name='br_conv1')
        br_conv2 = conv2d(br_conv1, output, kernel, stride,
                          relu=False, bn=False, name='br_conv2')
...
def residual_module(input_, output, is_BN, name):
    ...
    with tf.variable_scope(name) as scope:
        conv1 = conv2d(input_, mid_channel, 1, 1, relu=True,
                       bn=is_BN, name='res_conv1')
        conv2 = conv2d(conv1, mid_channel, 3, 1, relu=True,
                       bn=is_BN, name='res_conv2')
        conv3 = conv2d(conv2, output, 1, 1, relu=False,
                       bn=is_BN, name='res_conv3')
        conv_side = conv2d(input_, output, 1, 1, relu=False,
                           bn=is_BN, name='res_conv_side')
...
def gcn_residual_module(input_, output, gcn_kernel, is_BN, name):
    mid_channel = output >> 1
    with tf.variable_scope(name) as scope:
        gcn_layer = gcn(input_, mid_channel, gcn_kernel, 1,
                        relu=True, bn=is_BN, name='gcn_residual1')
        conv1 = conv2d(gcn_layer, output, 1, 1, relu=False,
                       bn=is_BN, name='gcn_residual2')
        conv_side = conv2d(input_, output, 1, 1, relu=False,
                           bn=-is_BN, name='gcn_residual3')
