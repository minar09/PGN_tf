...
def read_labeled_image_list(data_dir, data_list=None):
    """Reads txt file containing paths to images and ground truth masks.
    ...
    for image in image_list:
        ...
        # images.append(data_dir + image)
        # images.append(data_dir + "/image/" + image)
        images.append(os.path.join(image_dir, image))
        # masks.append(data_dir + mask)
        # masks.append(data_dir + "/image-parse/" + mask)
...
class ImageReader(object):
    ...
    def __init__(self, data_dir, data_list=None, data_id_list=None, input_size=None, random_scale=False,
                 random_mirror=False, shuffle=False, coord=None):
